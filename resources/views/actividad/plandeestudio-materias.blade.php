        ...
        <div class="row">
            <div class="col-lg-12">
                <ul class="nav nav-pills">
                  ...
                  <li class="nav-item">
                    <a class="nav-link active" href="/actividad/plan/{{ $globalId }}/materias">Materias</a>
                  </li>
                  <li class="nav-item">
                            ...
                            <div class="form-group col-lg-12">
                                ...
                                <select id="lstModulo" name="lstModulo" class="form-control selectpicker" data-live-search="true" onchange="abrirAltaModulo();">
                                    ...
                                    @for ($i = 0; $i < count($array_modulo); $i++)
                                        <option value="{{ $array_modulo[$i]->idgrupo }}">{{ $array_modulo[$i]->descgrupo }}</option>
                                    @endfor
                                </select>
                            ...
                            <div class="form-group col-lg-12">
                                ...
                                <select id="lstMateria" name="lstMateria" class="form-control selectpicker" data-live-search="true" onchange="abrirAltaMateria();">
                                    ...
                                    @for ($i = 0; $i < count($array_materia); $i++)
                                        <option value="{{ $array_materia[$i]->idmateria }}">{{ $array_materia[$i]->idmateria . " - " . ucfirst(strtolower($array_materia[$i]->descmateria)) }}</option>
                                    @endfor
                                </select>
                    ...
                    <div class="modal-body">
                        <form id="form-modulo">
                            <div class="row">
                                ...
                                @if($plandeestudio->abreinscripcion != 0)
                               <div class="form-group col-lg-6">
                        ...
                        <div class="row">
                            <div class="form-group col-lg-12">
                                ...
                                <select id="lstPlanEquivalente" name="lstPlanEquivalente" class="form-control" onchange="cargaMateriasEnSelectPorPlan();">
                                    ...
                                    @if($array_plan[$i]->idplan != $plandeestudio->idplan)
                                        <option value="{{ $array_plan[$i]->idplan }}" title="{{ $array_plan[$i]->descplan }}">{{ $array_plan[$i]->idplan . " - " . $array_plan[$i]->descplan }}</option>
                                    @endif
                                    @endfor
                                </select>
                            </div>
...
<script>
    ...
    function cargarGrilla() {
           ...
           /* "scrollY": "600px",
            "scrollCollapse": true,*/
            "paging": false,
            "ajax": "{{ asset('/actividad/plan/materias/cargarGrillaMateriasDelPlan?idplan='.$globalId) }}",
    ...
    function cargarGrillaEquivalenciaPorMateria(idMateria){
		...
		if(jsonPlanMat != "" && jsonPlanMat != undefined){
			...
			$.each(objPlanMat, function(index, value){
				...
				$.ajax({
		            type: "GET",
		            url: "{{ asset('actividad/materia/obtenerTodosPorPlanAjax') }}",
		            async: false,
		            dataType: "json",
					data: { 
						plan: idPlan
    ...
    function agregarMateriasEnGrilla() {
        ...
        @endphp
        var option = "@php echo $option; @endphp";
    ...
    function obtenerMateriasDelPlan(){
        ...
        $.ajax({
            type: "GET",
            url: "{{ asset('actividad/plan/materia/obtenerMateriasDelPlan') }}",
            data: { 
                    plan:globalId
                 },
            async: false,
    ...
    function agregarModuloEnSelect(){
		...
		if(modulo != "" && descripcion != ""){
        	$.ajax({
	            type: "GET",
	            url: "{{ asset('actividad/modulo/agregarModuloAjax') }}",
	            data: { 
		            	modulo: modulo,
    ...
    function frecargarModulosEnSelect(idModulo){
        $.ajax({
            type: "GET",
            url: "{{ asset('actividad/modulo/obtenerTodosPorPlanAjax') }}",
            async: true,
            dataType: "json",
            data: { 
                        plan:globalId
    ...
    function frecargarMateriasEnSelect(idMateria){
        $.ajax({
            type: "GET",
            url: "{{ asset('actividad/materia/obtenerTodosPorActividadDelPlanAjax') }}",
            async: true,
            dataType: "json",
            data: { 
                        plan: globalId
    ...
    function agregarMateriaEnSelect(){
		...
		if(descmateria != "" && ncmateria != "" && actividad != ""){
        	$.ajax({
	            type: "GET",
	            url: "{{ asset('actividad/materia/agregarMateriaAjax') }}",
	            data: { 
    ...
    function modificarMaterias(){
        ...
        if(descmateria != "" && ncmateria != "" && actividad != ""){
            $.ajax({
                type: "GET",
                url: "{{ asset('actividad/materia/modificarMateriaAjax') }}",
                data: { 
                        idmateria: idmateria,
                        descmateria: descmateria,
                        dpto: dpto,
    ...
    function abrirEditarMateria(idMateria){
        $.ajax({
            type: "GET",
            url: "{{ asset('actividad/materia/buscarMateria') }}",
            async: true,
            dataType: "json",
            data: { 
                    materia: idMateria
