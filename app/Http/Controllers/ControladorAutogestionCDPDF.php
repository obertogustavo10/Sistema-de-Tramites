...
class ControladorAutogestionCDPDF extends Controller
{
    public function generarComprobante($idformu)
    {
        if(Session::get('alumno_id')){
    		$alumno = new Alumno();
    		$alumno->obtenerPorId(Session::get('alumno_id'));
            ...
            $ultimoComprobante = $entidadCabecera->obtenerUltimoComprobantePorPlanPorAlumno(Session::get('usuarioalu'), $formuCabecera->fk_idplan);

            //Se fija que la constancia sea del alumno logueado y que sea el ultimo comprobante emitido
            if($formuCabecera && $formuCabecera->documento == Session::get('usuarioalu') && $ultimoComprobante->idformu == $idformu){
